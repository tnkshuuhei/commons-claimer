...
export default function InsightPage() {
    ...
    async queryFn() {
      try {
        return await request(
          "https://indexer.bigdevenergy.link/92f8c3e/v1/graphql/",
          QUERY_DOCUMENT
        ).then((data: any) => data.Account);
      } catch (error) {
        console.error(error);
  ...
  const chartData = {
    labels: balancesWithoutZeroAddress.map(
      (account) => `${account.id.slice(0, 6)}...${account.id.slice(-4)}`
    ),
    datasets: [
      {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">Common Holders</h1>
      <WalletHolders accounts={accounts} />

      <h1 className="text-3xl font-bold mb-8">Account Insights</h1>

      <div className="mb-4 flex space-x-4">
        <Select
          onValueChange={(value: "balance" | "stakedNFTs") => setSortBy(value)}
        >
    ...
    <Card className="hover:shadow-lg transition-shadow duration-300">
      ...
      <CardContent className="pt-4">
        <p className="text-2xl font-bold mb-2 text-indigo-600">
          {`${parseFloat(formatEther(BigInt(account.balance))).toFixed(
            2
          )} $COMMONS`}
        </p>
        <p className="text-sm text-gray-600 mb-2">
          Staked NFTs:{" "}
          <span className="font-semibold">{account.stakedNFTs.length}</span>
        </p>
        {account.stakedNFTs.length > 0 && (
          <div className="bg-gray-100 p-3 rounded-md">
            ...
            <p className="text-sm">
              ...
              {account.stakedNFTs[0].tokenId}
            </p>
            <p className="text-sm">
              <span className="font-semibold">Amount:</span>{" "}
              {formatEther(BigInt(account.stakedNFTs[0].amount))} $COMMONS
            </p>
            <p className="text-sm">
              <span className="font-semibold">Status:</span>
              <span
                className={`ml-1 px-2 py-1 rounded-full text-xs ${
                  account.stakedNFTs[0].active
            ...
            <p className="text-sm">
              ...
              {new Date(
                parseInt(account.stakedNFTs[0].timestamp) * 1000
              ).toLocaleString()}
            </p>
          </div>
        )}
